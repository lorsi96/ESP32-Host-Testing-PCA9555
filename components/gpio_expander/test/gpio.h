...
#define GPIO_IS_VALID_GPIO(gpio_num) \
  ((gpio_num < GPIO_PIN_COUNT && GPIO_PIN_MUX_REG[gpio_num] != 0)) /*!< Check whether it is a valid GPIO number */
#define GPIO_IS_VALID_OUTPUT_GPIO(gpio_num)                                                                           \
  ((GPIO_IS_VALID_GPIO(gpio_num)) && (gpio_num < 34)) /*!< Check whether it can be a valid GPIO number of output mode \
                                                       */
 ...
 *
 * @param  gpio_num GPIO number. If you want to set the trigger type of e.g. of GPIO16, gpio_num should be GPIO_NUM_16
 * (16);
 ...
 *
 * @param  gpio_num GPIO number. If you want to disable the interrupt of e.g. GPIO16, gpio_num should be GPIO_NUM_16
 * (16);
 *
 * @return
 *     - ESP_OK success
 ...
 *
 * Only pins that support both input & output have integrated pull-up and pull-down resistors. Input-only GPIOs 34-39 do
 * not.
 *
 ...
 * To disable or remove the ISR, pass the returned handle to the :doc:`interrupt allocation functions
 * </api-reference/system/intr_alloc>`. \endverbatim
 *
 * @return
 *     - ESP_OK Success ;
 *     - ESP_ERR_INVALID_ARG GPIO error
 ...
 *
 * When the chip is in Deep-sleep mode, all digital gpio will hold the state before sleep, and when the chip is woken
 * up, the status of digital gpio will not be held. Note that the pad hold feature only works when the chip is in
 * Deep-sleep mode, when not in sleep mode, the digital gpio state can be changed even you have called this function.
 *
 * Power down or call gpio_hold_dis will disable this function, otherwise, the digital gpio hold feature works as long
 * as the chip enter Deep-sleep.
 */
